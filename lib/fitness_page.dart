...
class ChatMessage {
  ...
  Map<String, dynamic> toJson() => {
    ...
    'styleboard': styleboard?.toJson(),
    'checklistItems': checklistItems,
    'workoutCards': workoutCards.map((c) => c.toJson()).toList(),
  };

  factory ChatMessage.fromJson(Map<String, dynamic> j) => ChatMessage(
    text: (j['text'] ?? '').toString(),
    isBot: j['isBot'] == true,
    isGreeting: j['isGreeting'] == true,
    time: DateTime.tryParse(j['time'] ?? '') ?? DateTime.now(),
    styleboard: j['styleboard'] != null
        ? WorkoutStyleboard.fromJson(Map<String, dynamic>.from(j['styleboard'] as Map))
        : null,
    checklistItems: ((j['checklistItems'] as List?) ?? const [])
        .map((e) => e.toString())
        .toList(),
    workoutCards: ((j['workoutCards'] as List?) ?? const [])
        .map((e) => AhviWorkoutCard.fromMap(Map<String, dynamic>.from(e as Map)))
        .toList(),
  );
}
...
class WorkoutStyleboard {
  ...
  factory WorkoutStyleboard.fromJson(Map<String, dynamic> j) =>
      WorkoutStyleboard(
        ...
        colors: ((j['colors'] as List?) ?? const [])
            ...
            .toList(),
        gradientColors: ((j['gradientColors'] as List?) ?? const [])
            .map((v) => Color(v as int))
            .toList(),
      );
}
...
class WorkoutStudioScreen extends StatefulWidget {
  final bool fromHome;
  const WorkoutStudioScreen({super.key, this.fromHome = false});
  @override
...
class _WorkoutStudioScreenState extends State<WorkoutStudioScreen> {
  ...
  Future<void> _loadTodayWorkout() async {    if (!mounted) { debugPrint('AHVI_WORKOUT_TODAY_SKIPPED reason=unmounted'); return; }    if (_todayWorkoutLoading) { debugPrint('AHVI_WORKOUT_TODAY_SKIPPED reason=already_loading'); return; }    if (_todayWorkoutLoaded) { debugPrint('AHVI_WORKOUT_TODAY_SKIPPED reason=already_loaded'); return; }    setState(() => _todayWorkoutLoading = true);    try {      final appwrite = Provider.of<AppwriteService>(context, listen: false);      final backend = Provider.of<BackendService>(context, listen: false);      final profile = await appwrite.refreshCurrentUserProfile();      if (!mounted) return;      final gender = (profile?['gender'] ?? '').toString().toLowerCase();      final response = await backend.getTodayWorkout();      if (!mounted) return;      final cards = _workoutCardsFromResponse(response)          .map((card) => _personalizedWorkoutCard(card, gender))          .toList(growable: false);      setState(() {        _todayWorkout = cards.isNotEmpty ? cards.first : null;        _todayWorkoutLoading = false;        _todayWorkoutLoaded = true;      });    } catch (e) {      debugPrint('AHVI_WORKOUT_TODAY_STATUS status=error content_type= error=$e');      if (!mounted) return;      setState(() {        _todayWorkout = null;        _todayWorkoutLoading = false;        _todayWorkoutLoaded = true;      });    }  }
...
class _HomeView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        // ── Full-width Page Header (shown only when fromHome = true) ────
        if (fromHome)
          Container(
            ...
            child: Row(
              children: [
                ...
                Text(
                  'Fitness',
                  style: TextStyle(
                    color: context.fIsDark ? Colors.white : Colors.black,
                    fontSize: 20,
...
class _FitnessSession {
  ...
  factory _FitnessSession.fromJson(Map<String, dynamic> j) => _FitnessSession(
    id: j['id'] as String,
...
class _ChatViewState extends State<_ChatView> {
  ...
  static const _historyStorageKey = 'ahvi_fitness_chat_history';
  ...
  Future<void> _loadHistoryFromDisk() async {
    ...
    try {
      ...
      final loaded = decoded
          .map((e) => _FitnessSession.fromJson(Map<String, dynamic>.from(e as Map)))
          .toList()
      ...
      // Corrupt/old-shape data — ignore rather than crash the page.
    }
  }
  ...
  void didChangeDependencies() {
    ...
    if (_messages.isEmpty) {
      _messages.add(
        ChatMessage(
          text: AppLocalizations.t(context, 'fitness_chat_greeting'),
          isBot: true,
          isGreeting: true,
        ),
      );
  ...
  @override
  void dispose() {
    _saveCurrentSession(); // safety net if the page is left mid-chat
    if (_isListening) {
