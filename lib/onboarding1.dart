...
class _Screen1State extends State<Screen1> with TickerProviderStateMixin {
  ...
  OverlayEntry? countryDropdownOverlay;
  final LayerLink _countryLayerLink = LayerLink();

  void _showCountryPicker() {
    if (countryDropdownOverlay != null) {
      _removeCountryDropdown();
      return;
    }
    final overlay = Overlay.of(context);
    countryDropdownOverlay = OverlayEntry(
    ...
    overlay.insert(countryDropdownOverlay!);
  }

  void _removeCountryDropdown() {
    countryDropdownOverlay?.remove();
    countryDropdownOverlay = null;
  }
  ...
  Future<void> _onContinue() async {
    ...
    if (!_isValid) {
      _showValidationError(
        'Please complete your name, gender, date of birth, and shop preferences.',
      );
      return;
    }
    ...
    try {
      ...
      if (currentUser != null) {
        debugPrint(
          'AHVI_SIGNUP_PROFILE_SAVE userId=${currentUser.$id} gender=${gender.toLowerCase()} collection=users status=success',
        );
        final profileDoc = await appwrite.refreshCurrentUserProfile();
        debugPrint(
          'AHVI_SIGNUP_PROFILE_VERIFY read_gender=${profileDoc?['gender']}',
        );
      }
        ...
        children: [
          ...
          Stack(
            ...
            children: [
              ...
              const Positioned(
                right: 14,
                child: Text('', style: TextStyle(fontSize: 12, color: accent2)),
              ),
            ],
          ),
        ],
