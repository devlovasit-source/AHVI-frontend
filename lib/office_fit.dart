...
class _OfficeFitScreenState extends State<OfficeFitScreen> {
  ...
  Future<void> _fetchLooks() async {
    try {
      ...
      final docs = await appwrite.getSavedBoardsByOccasion('Office');
      ...
      for (var doc in docs) {
        ...
        final category =
            (doc.data['boardCategoryLabel'] ?? 'Office').toString();

        loadedLooks.add(
          LookItem(
            id: doc.$id,
            title: (doc.data['title'] ?? doc.data['occasion'] ?? 'Office')
                .toString(),
            description: (doc.data['outfitDescription'] ??
                    'Custom Office inspiration')
                .toString(),
