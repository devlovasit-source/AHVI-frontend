...
class _PartyLooksScreenState extends State<PartyLooksScreen> {
  ...
  Future<void> _fetchLooks() async {
    try {
      ...
      final docs = await appwrite.getSavedBoardsByOccasion('Party');
      ...
      for (var doc in docs) {
        ...
        final category =
            (doc.data['boardCategoryLabel'] ?? 'Party').toString();

        loadedLooks.add(
          LookItem(
            id: doc.$id,
            title: (doc.data['title'] ?? doc.data['occasion'] ?? 'Party')
                .toString(),
            description: (doc.data['outfitDescription'] ??
                    'Custom Party inspiration')
                .toString(),
