...
class _DailyWearScreenState extends State<DailyWearScreen>
  ...
  Future<void> _fetchWeather() async {
    ...
    try {
      ...
      if (weather == null) {
        debugPrint(
          'AHVI_HEAVY_SCREEN_LOAD weather unavailable screen=DailyWear',
        );

        // Keep the existing fallback behavior when location/weather
        // cannot be obtained.
      ...
      final feel = weather.apparentTemperature.round();
      final code = weather.weatherCode;

      _applyWeather(temp, feel, code);

      debugPrint(
        '🌦️ Daily Wear weather: '
        '${temp}°C '
        'feels like ${feel}°C '
        'at ${weather.latitude}, ${weather.longitude}',
      );
    } catch (e) {
      debugPrint(
        'AHVI_HEAVY_SCREEN_LOAD weather error '
        'screen=DailyWear: $e',
      );
