...
class _DailyWearScreenState extends State<DailyWearScreen>
  ...
  static const String _dailyBoardHistoryKey = 'daily_wear_board_history';

  Future<Set<String>> _readDailyBoardHistory() async {
  final prefs = await SharedPreferences.getInstance();
  final raw = prefs.getString(_dailyBoardHistoryKey);
  ...
  try {
    ...
    final historyDate = DateTime.tryParse(
  ...
  Future<void> _saveDailyBoardHistory(
    List<Map<String, dynamic>> outfits,
  ) async {
    final prefs = await SharedPreferences.getInstance();

    final boardIds = outfits
        .map((outfit) => outfit['id']?.toString() ?? '')
        .where((id) => id.isNotEmpty)
        .toList();

    await prefs.setString(
      _dailyBoardHistoryKey,
      jsonEncode({
  ...
  Map<String, dynamic> _normalizeDailyBoardCard(
    ...
    final items = card['items'] is List
        ? List<Map<String, dynamic>>.from(
            (card['items'] as List).whereType<Map>().map(
  ...
  bool _isInsufficientWardrobeResponse(Map<String, dynamic>? response) {
    ...
    final nestedIntent = data is Map
        ? data['intent']?.toString().toLowerCase().trim()
        : null;
  ...
  Future<void> _fetchDailyBoard() async {
    ...
    final previousBoardIds = await _readDailyBoardHistory();

    final filteredOutfits = outfits.where((outfit) {
      final boardId = outfit['id']?.toString() ?? '';
      return boardId.isEmpty || !previousBoardIds.contains(boardId);
    }).toList();
    ...
    await _saveDailyBoardHistory(filteredOutfits);

    setState(() {
      _applyOutfits(filteredOutfits);
      _isLoading = false;
      _needsMoreClothes = false;
      _emptyStateMessage = '';
    });
  ...
  Future<void> _persistCurrentLook() async {
    ...
    try {
      final saved = await AppwriteService().saveBoardToCollection(
        occasion: 'daily',
        outfitDescription: description.isEmpty
            ? 'AHVI curated daily drop'
            : description,
