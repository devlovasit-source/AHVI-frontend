...
class _MediTrackScreenState extends State<MediTrackScreen>
  ...
  Future<void> _scheduleMedicineReminder({
    ...
    final ok = await BackendService(appwriteService: appwrite).scheduleReminder(
      eventId: medId,

      message: 'Time to take $medName${dose.trim().isEmpty ? '' : ' - $dose'}.',
    ...
    debugPrint(
      'AHVI_MED_REMINDER_SCHEDULE_REQUEST medId=$medId ok=$ok sendAt=${sendAt.toUtc().toIso8601String()}',
    );
  }
      ...
      child: Row(
        ...
        children: [
          ...
          _buildIconBtn(
            ...
            badge: AhviNotificationService.instance.unreadCount.value > 0,
          ),
        ],
      ),
    );
  ...
  TimeOfDay _initialTimeFromText(String value) {
    ...
    if (meridiemMatch != null) {
      var hour = int.tryParse(meridiemMatch.group(1) ?? '');
    ...
    if (twentyFourMatch != null) {
      ...
      final minute = int.tryParse(twentyFourMatch.group(2) ?? '0') ?? 0;
  ...
  Future<void> _pickMedicineTime(StateSetter setSheetState) async {
    final selected = await showTimePicker(
      context: context,
      initialTime: _initialTimeFromText(_timeCtrl.text),
      builder: (context, child) {
    ...
    setSheetState(() {
      _timeCtrl.text = _formatTimeOfDay(selected);
    });
  }
  ...
  Widget _buildTextField(
    ...
    return Container(
      ...
      child: TextField(
        ...
        readOnly: ctrl == _timeCtrl,

        onTap: ctrl == _timeCtrl ? () => _pickMedicineTime(setState) : null,
