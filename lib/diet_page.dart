...
export 'package:myapp/models/meal_plan.dart';
...
// ─── DATA MODELS ─────────────────────────────────────────────────────────────
// Meal, DayPlan, MealPlan moved to models/meal_plan.dart (re-exported above).
...
class MainScreen extends StatefulWidget {
  final bool fromHome;
  const MainScreen({super.key, this.fromHome = false});
  @override
...
class _MainScreenState extends State<MainScreen> {
  ...
  late final DietPlansController _ctrl;
  List<MealPlan> get _plans => _ctrl.plans;
  ...
  void initState() {
    ...
    _ctrl = DietPlansController(DietPlansGateway(
      list: () async {
        final docs = await appwrite.getMealPlans();
        return docs
            .map((d) => mealPlanFromDocumentData({r'$id': d.$id, ...d.data}))
            .whereType<MealPlan>()
            .toList();
      },
      create: (payload) async {
        final doc = await appwrite.createMealPlan(payload);
        return doc.$id;
      },
      update: (id, payload) => appwrite.updateMealPlan(id, payload).then((_) {}),
      delete: (id) => appwrite.deleteMealPlan(id),
    ));
  ...
  /// Create: persist to Appwrite FIRST, add the card only on success so a
  /// failed write never leaves a fake local plan.
  Future<void> _addPlan(MealPlan p) async {
    ...
    try {
      await _ctrl.create(draft);
      if (!mounted) return;
      setState(() {});
    } catch (e) {
      debugPrint('Create meal plan failed: $e');
      if (!mounted) return;
      _showSnack(_errSnack('Could not save "${p.name}". Please try again.'));
    }
  }

  Future<void> _savePlanFromChat(MealPlan p) async {
    final draft = MealPlan(
      id: DateTime.now().millisecondsSinceEpoch,
      name: p.name,
      desc: p.desc,
    ...
    try {
      final saved = await _ctrl.create(draft);
      if (!mounted) return;
      setState(() {});
      _showSnack(
        SnackBar(
          content: Row(
            children: [
              ...
              Expanded(
                child: Text(
                  '"${saved.name}" ${AppLocalizations.t(context, 'diet_saved_to_plans')}',
      ...
      debugPrint('Save-from-chat meal plan failed: $e');
      if (!mounted) return;
      _showSnack(_errSnack('Could not save "${p.name}". Please try again.'));
    }
  }
  ...
  Future<void> _deletePlan(int id) async {
    ...
    if (plan == null) return;
    // Not yet persisted (no Appwrite row) -> just drop the local card.
    if (plan.documentId == null || plan.documentId!.isEmpty) {
      setState(() => _ctrl.plans.removeWhere((p) => p.id == id));
      return;
    }
    final target = plan;
    try {
      await _ctrl.delete(target);
      if (!mounted) return;
      setState(() {});
    } catch (e) {
      debugPrint('Delete meal plan failed: $e');
      if (!mounted) return;
      _showSnack(_errSnack(
          'Could not delete "${target.name}". Please try again.'));
    }
  }

  /// Edit: write to Appwrite FIRST; replace the local card and show success
  /// only after the update is confirmed. documentId is authoritative.
  Future<void> _editPlan(MealPlan updated) async {
    if (updated.documentId == null || updated.documentId!.isEmpty) {
      debugPrint('Edit blocked: missing documentId for "${updated.name}"');
      _showSnack(_errSnack('Cannot update "${updated.name}" - not saved yet.'));
      return;
    }
    try {
      await _ctrl.edit(updated);
      if (!mounted) return;
      setState(() {});
      _showSnack(
        SnackBar(
      ...
      debugPrint('Update meal plan failed (Appwrite): $e');
      if (!mounted) return;
      _showSnack(_errSnack(
          'Could not update "${updated.name}". Please try again.'));
    }
  }
    ...
    return ScaffoldMessenger(
      ...
      child: Scaffold(
        ...
        body: SafeArea(
          child: Column(
            children: [
              // ── Page Header (shown only when fromHome = true) ──────────────
              if (widget.fromHome)
                Container(
                      ...
                      GestureDetector(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            color: context.dAccent.withValues(alpha: 0.12),
                            borderRadius: BorderRadius.circular(10),
                            border: Border.all(
                              color: context.dAccent.withValues(alpha: 0.25),
                            ),
                          ),
                          child: Icon(
              ...
              Expanded(
                child: widget.loadError != null
                    ? _loadErrorState()
                    : (widget.loading && widget.plans.isEmpty)
  ...
  Widget _loadErrorState() {
    return Center(
      child: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          Icon(Icons.cloud_off_rounded, size: 40, color: context.dMuted),
          const SizedBox(height: 12),
          Text('Could not load your plans',
              style: TextStyle(
                  fontWeight: FontWeight.w700, color: context.dText)),
          const SizedBox(height: 4),
          Text('Check your connection and retry.',
              style: TextStyle(color: context.dMuted)),
          const SizedBox(height: 12),
...
class _AddMealModalState extends State<AddMealModal> {
  ...
  void _save() {
    ...
    // Success is confirmed by the parent only after Appwrite persists.
  }
...
class _EditMealModalState extends State<EditMealModal> {
  ...
  void _save() {
    ...
    widget.onSave(
      MealPlan(
        id: widget.plan.id,
        documentId: widget.plan.documentId,
        name: name,
